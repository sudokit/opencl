...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        ...
        self.image_item.setLookupTable(
            pg.colormap.getFromMatplotlib("jet").getLookupTable()
        )
        self.plot_item.addItem(self.image_item)

        self.color_legend_item = ColorLegendItem(
            imageItem=self.image_item, showHistogram=True, label="Heat distribution"
        )
        ...
        self.plot_item.setRange(
            xRange=[0, self.sim.dims.width], yRange=[0, self.sim.dims.height]
        )
        self.video_writer = cv2.VideoWriter(
            "./out/woop.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 60, (width, height)
        )
    ...
    def update_plot(self):
        ...
        if dt != 0:
            ...
            if self.fps > FPS:
                sleep(dt)
        self.setWindowTitle(
            f"{self.fps:.3f} fps | {dt:.4f} dt | {'paused' if self.paused else 'running'}"
        )
